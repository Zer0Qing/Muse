 ...
 *
 * 参考实现:rikkahub ConversationSession。
 *
 * 设计目的:
 ...
 *  idle 检查协程运行在构造时传入的 [scope] 中。
 *
 * @param scope idle 检查协程运行的 CoroutineScope(应使用应用级 appScope,避免随 ViewModel 生命周期取消)
 */
class ConversationSessionManager(
    ...
    private val idleCheckJobs = ConcurrentHashMap<String, Job>()
    ...
    fun acquire(sessionId: String) {
        ...
        idleCheckJobs[sessionId]?.cancel()
    }
     ...
     * 引用计数归零时调度 idle 检查(等 [IDLE_TIMEOUT_MS] 后清理,留出短时间切回的窗口)。
     */
    fun release(sessionId: String) {
        val ref = sessions[sessionId] ?: return
        val count = ref.refCount.decrementAndGet()
        if (count <= 0) {
    ...
    /**
     * 调度 idle 检查:延迟 [IDLE_TIMEOUT_MS] 后,若该会话引用计数仍 ≤ 0 且无生成任务,
     * 则从 [sessions] 移除,释放内存。
     *
     * 同一会话已有 idle 检查时先取消旧的,保证至多一个 pending。
     */
    private fun scheduleIdleCheck(sessionId: String) {
        idleCheckJobs[sessionId]?.cancel()
        idleCheckJobs[sessionId] = scope.launch {
            delay(IDLE_TIMEOUT_MS)
            val ref = sessions[sessionId]
            if (ref != null && ref.refCount.get() <= 0 && ref.generationJob == null) {
                sessions.remove(sessionId)
                idleCheckJobs.remove(sessionId)
            }
        }
    }
     ...
     * 清理所有会话(App 退出 / 测试用)。
     *
     * 取消所有生成任务与 idle 检查协程,清空 [sessions]。
     */
    fun clearAll() {
