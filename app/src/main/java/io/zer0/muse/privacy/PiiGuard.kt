...
object PiiGuard {
    ...
    private val patterns: Map<PiiType, Regex> = linkedMapOf(
        ...
        PiiType.ADDRESS to Regex(
            // B-01/A-20 防误判: 移除裸"号"关键字 — "卡号 6222…"/"单号"等非地址文本
            // 会以"号"为锚吞掉整串数字,导致 16-19 位卡号被标成 ADDRESS 而不走 BANK_CARD。
            // 地址中的"号"通常跟在路/街/巷/弄之后,由前置关键字命中即可。
            "(?:省|市|区|县|镇|乡|村|路|街|巷|弄|室|栋|单元)\\s*\\d{1,6}(?:[^-\\s]{0,20})",
        ),
    )
