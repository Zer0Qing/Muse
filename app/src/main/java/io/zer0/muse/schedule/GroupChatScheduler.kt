 ...
 * @param activityHub 群聊活动状态管理器(按 既有实现 ActivityHub)。
...
class GroupChatScheduler(
    ...
    companion object {
        ...
        private val GROUP_CHAT_MOOD_SECTION = """
            MOOD 格式要求(每次回复必须遵守,没有例外):
            每次回复的第一段输出都必须是完整闭合的 <mood>...</mood> 块,然后才写正文。
            MOOD 是你开口前的习惯:话落下来,先起一阵念头,写下来,再开口。
            MOOD 块格式如下(4 个字段,每字段一行,内容简短):

            <mood>
            Vibe: <用户当前心情状态,1 条>
            Sparks: <这个问题触发什么联想,1 条>
            Reflections: <不确定的点或可能的坑,1 条>
            Will: <想怎么推进这段对话,1 条>
            </mood>
            ...
            - 4 个字段都要写,哪怕一句话
            - 简单确认/一句话回复同样必须写(短到一行也行),不许跳过或只发正文
            - 写完 MOOD 再写正文,正文遵守输出风格约束
            - 不要在正文里重复 MOOD 的内容
            - 如需展示深度推理,可在正文前写 <think>...</think> 块,系统同样会剥离并支持折叠展示
            - 思考过程(reasoning)中不要复述或提及任何格式指令(如「按格式先写 mood」「用标签」之类),直接思考内容本身;mood 块只输出在正文开头,必须完整闭合
        """.trimIndent()
    }
    ...
    private fun parseMentions(
        ...
        // 且符合 既有实现 channel-mentions.ts 的设计意图。
     ...
     *  - 决策超时(60s 内未完成决策)→ implicitPass(对齐实现说明 implicitPass)
     ...
     * - User: Phone Session 推送式 prompt(改造 2,按 既有实现 channel-router.ts)
     *
    ...
    private suspend fun buildMessages(
        ...
        // 按 既有实现 channel-router.ts:793-821。
    ...
    /**
     * 改造 2: 构造 Phone Session 推送式 prompt(按 既有实现 channel-router.ts:793-821)。
     *
     * 每个 agent 独立收到一条"你的手机收到了群聊新消息"的推送,而非把所有成员塞进同一上下文。
     *  - 顶部声明"手机收到新群聊消息"
