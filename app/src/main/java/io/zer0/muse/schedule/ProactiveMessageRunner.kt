...
class ProactiveMessageRunner(
     ...
     *
     * [triggerSource] 影响:
     *  - "poll" / "worker":受 [computeEffectiveIntervalMs] 间隔限制
    ...
    private suspend fun executeProactiveCycle(
        ...
        // v2.0 5.6: 长时间沉默事件触发校验 — 仅当 elapsed > interval × 1.5 时才允许 long_silence 事件触发
        if (triggerSource == TRIGGER_SOURCE_LONG_SILENCE && elapsed < effectiveIntervalMs * 1.5f) {
