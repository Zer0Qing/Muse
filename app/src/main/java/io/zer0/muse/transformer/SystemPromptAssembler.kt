...
class SystemPromptAssembler(
     ...
     * 参考 openhanako:群聊消息摘要写入独立 fact store(本仓库),不进入主记忆系统。
     * 为 null 时不注入(测试环境或未注入时降级)。
     */
...
- 如果用户请求违法、侵害隐私或明显有害的操作,拒绝并解释原因
        ...
        fun buildGroupChatHintSection(chatName: String, members: List<String>, currentAgentName: String): String {
            ...
            // 改造 3: 注入身份防混淆 guidance(per-agent,参考 openhanako _formatChannelIdentityGuidance)
        ...
        /**
         * 改造 3: 身份防混淆 guidance(参考 openhanako _formatChannelIdentityGuidance)。
         *
