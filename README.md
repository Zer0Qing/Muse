...
Muse 是 Android 上最懂你的 AI 伴侣。它通过四层记忆系统**真正记住你是谁**——你的偏好、你的习惯、你在意的事。换模型、切会话、关掉重开，它都记得。

自带内心独白（Mood 机制），每次回复前写下四个维度的思考过程。这不是一串冰冷的日志——是 AI 此刻的情绪、脑中闪过的联想、对自己的反思。默认折叠不打扰，展开一看挺有意思。

**你的选择很自由：**
- 模型随便换：OpenAI、Anthropic、Gemini、DeepSeek，或者任意 OpenAI 兼容接口
- 不用注册、无需账号、数据默认只在本地
- 能联网、能说话、能执行工具、能搜索
- 支持群聊、Agent 多角色协作
- 甚至在你久未联系时会主动发起对话

一切为了一个目的：让 AI 真正**延续**你们的对话，而不是每次都从零开始。
